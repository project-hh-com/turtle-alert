const {
  initDetector,
  captureAndAnalyze: _rawCaptureAndAnalyze,
  captureCalibrationFrames,
  disposeDetector,
  CAPTURE_WIDTH,
  CAPTURE_HEIGHT,
  CALIBRATION_FRAME_COUNT,
} = require("./posture-capture");
...
/**
 * 카메라 촬영 → 포즈 추정 → 자세 판정 (evaluatePosture 바인딩)
 * @param {object|null} baseline
 * @returns {Promise<{isGood: boolean, issues: string[]}>}
 */
function captureAndAnalyze(baseline) {
  return _rawCaptureAndAnalyze(evaluatePosture, baseline);
}
